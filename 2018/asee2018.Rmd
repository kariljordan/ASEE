...
geom_bar(stat = "identity", position = "dodge", fill = "blue", na.rm = TRUE ) +
...
round2data %>%
  ...
  ggplot(pre_and_post_usage, aes(x = factor(value, levels = programming), y = perc, fill = key)) +
...
ggplot(stdres,
  ...
  xlab("Figure 5: Respondents program significantly more often") +
...
data_change_in_confidence_tally <-
  round2data %>%
...
# kable(data_change_in_confidence_tally, format = "markdown", row.names = FALSE, col.names = c("Change in Confidence", "%"))

ggplot(data_change_in_confidence_tally,
   ...
   ylab("% Respondents") +
   ggtitle("Figure 5: Change in Confidence") +
   theme(plot.title = element_text(hjust = 0.5)) +
...
```{r}
status <- unique(round2data$Status)
...
```{r}
status <- factor(status, levels(status)[c(1, 2, 3, 4, 6, 7, 5)])
# Display ordering.
print(levels(status))

round2data$Status <-
  factor(round2data$Status, levels = levels(status))

change_in_confidence_by_status <- round2data %>%
  group_by(ChangeInConfidence, Status) %>%
  tally() %>%
  filter(!is.na(ChangeInConfidence)) %>%
  group_by(Status) %>%
...
ggplot(data = change_in_confidence_by_status,
       aes(x = ChangeInConfidence, y = Status)) +
  ...
  scale_fill_gradient("Percent",
                      ...
                      high = "blue",
                      limits = c(0, 100)) +
  xlab("Figure 7: Change in Confidence by Status") +
  theme(
...
<<<<<<< HEAD
   ...
   geom_bar(stat = "identity", fill = "blue") +
=======
       aes(ChangeInConfidence, y = perc, n)) +
   geom_bar(stat = "identity", fill = "grey") +
>>>>>>> 81cc6d6983cb5010a0b229550a23cebd0641e03a
   geom_text(aes(label=n), size= 4, vjust=-0.25) +
...
```{r}
# Breakdown by status.
change_in_conf_eng_by_status <- round2data %>%
  filter(grepl("Engineering", Column6)) %>%
  group_by(ChangeInConfidence, Status) %>%
  tally() %>%
  filter(!is.na(ChangeInConfidence)) %>%
...
# Check that percentage total is 100 for each status.
...
title <- ""
plot(data_agree_likert, type =c("heat"), panel.arrange = NULL, panel.strip.color = "blue", legend.position = "bottom") + ggtitle(title) + labs(x = "Factor", y="Level of Agreement")
...
kable(round1data_country_tally)
```
